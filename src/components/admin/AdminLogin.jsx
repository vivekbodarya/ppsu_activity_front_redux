...
import { REQ_FOR_LOGIN_PROGRESS_ADMIN } from '../../redux/login/admin/action';
...
const AdminLogin = () => {
    ...
    const onSubmit = (data) => {
        console.log(data)
        dispatch({ type: REQ_FOR_LOGIN_PROGRESS_ADMIN, payload: { data } })
        setStatus(true)
    }
    // This is for validation and message show for End-User
    if (adminLogin.login_progress === false && adminLogin.login_auth_failed === true && status === true) {
        errHandle("Auth Failed! 🤦‍♂️")
        setStatus(false)
    }
    if (adminLogin.login_progress === false && adminLogin.login_status_success === true && status === true) {
        successHandle("Auth Success! 🤦‍♂️")
        setStatus(false)
    }
    if (adminLogin.login_progress === false && adminLogin.login_error === true && status === true) {
                    ...
                    <div className="col-sm-12 col-md-8 col-lg-6">
                        <div className='mt-5 mb-5 login_section'>
                            ...
                            <form className='p-5' onSubmit={handleSubmit(onSubmit)}>
                                ...
                                {
                                    adminLogin.login_progress === true ?
